...
def test_collection_rejects_from_memtable(cql, test_keyspace):
    ...
    The first write (insertion) is not tracked for collections because
    coordinator-side guardrails handle that case.  The second write (merge)
    caches the count, and the third write is rejected."""
    with ExitStack() as cfg:
        cfg.enter_context(config_value_context(cql,
            'compaction_collection_elements_count_warning_threshold', '50'))
        cfg.enter_context(config_value_context(cql,
        ...
        with new_test_table(cql, test_keyspace, schema,
            ...
            insert = cql.prepare(f"INSERT INTO {tbl} (pk, ck, s) VALUES (?, ?, ?)")
            update = cql.prepare(f"UPDATE {tbl} SET s = s + ? WHERE pk = ? AND ck = ?")
            # Write 1: insertion (miss) — succeeds, collection not cached.
            cql.execute(insert, [1, 0, set(range(60))])
            # Write 2: UPDATE adds to the set (hit) — on_collection_merged
            # caches count 61, but check() ran before apply() so this passes.
            cql.execute(update, [{999}, 1, 0])

            # Write 3: check() finds cached count → rejected.
            with pytest.raises(WriteFailure, match=REJECT_COLLECTION_RE):
...
def test_collection_warns_from_memtable(cql, test_keyspace, logfile):
    ...
    The second write (merge) caches the count, and the third write
    ...
    with ExitStack() as cfg:
        ...
        with new_test_table(cql, test_keyspace, schema,
            ...
            insert = cql.prepare(f"INSERT INTO {tbl} (pk, ck, s) VALUES (?, ?, ?)")
            update = cql.prepare(f"UPDATE {tbl} SET s = s + ? WHERE pk = ? AND ck = ?")
            # Write 1: insertion (miss) — collection not cached.
            cql.execute(insert, [1, 0, set(range(60))])
            # Write 2: UPDATE adds to the set (hit) — caches count.
            cql.execute(update, [{999}, 1, 0])

            # Write 3: check() finds cached count → warning.
            cql.execute(update, [{998}, 1, 0])
